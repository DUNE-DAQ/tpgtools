...
void save_TP_object( triggeralgs::TriggerPrimitive trigprim, std::string algo, 
		     std::string path_output, std::string out_suffix ){
  ...
  std::string file_name = path_output + "/" + "TP_dump_" + algo + "_" + date_time_str + out_suffix + ".txt";
           ...
           std::string algo)
{
  ...
  if (channel_number == -1) {
    ...
    file_name = "Channel_" + std::to_string(channel_number) + "_" + "_data" + date_time_str + ".txt";
  }
  out_file.open(file_name.c_str(), std::ofstream::app);
